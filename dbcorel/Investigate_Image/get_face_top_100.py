...
def findImageInTree_DBCOREL(image):
	start = time.time()
	lstImage = []
	...
	print 'time compute query image:', end - start
	lstVector = []
	...
	with open("tree/root.txt") as f:
		for line in f:
			...
			vec = Vector(v, temp[0])
			lstVector.append(vec)
	dirMin = findMinDistance(arrVector, lstVector)
	link = lstVector[dirMin].getLink()
	tempFile = open("tree/" + str(link) + ".txt")
	model = tempFile.readline()
	...
	while True:
		...
		if model.__contains__('Node'):
			...
			with open('tree/' + str(link) + '.txt') as f:
				for line in f:
					...
					q = Vector(tempVector, temp[0])
					lstV.append(q)
				dirMin = findMinDistance(arrVector, lstV)
				link = lstV[dirMin].getLink()
				tempFile = open('tree/' + str(link) + '.txt')
				model = tempFile.readline()
		...
		if model.__contains__('Leaf'):
			...
			with open("tree/" + str(link) + ".txt") as f:
				...
				lstDistance = []
				for i in range(0, len(lstV)):
					lstDistance.append(computeDistanceEuclide(lstV[i].getVector(), arrVector))
				print lstDistance
				for i in range(0, len(lstDistance) - 1):
					for j in range(i + 1, len(lstDistance)):
						if lstDistance[i] > lstDistance[j]:
	...
	print 'time find image in tree', end - start
	return lstImage