...
def findImageInTree(vector):
	...
	# tempFile = open("tree/" + str(lstVector[0].getLink()) + ".txt")
	...
	while True:
		isFirstLine = True
		with open("tree/" + str(lstVector[currentLocation].getLink()) + ".txt") as f:
			for line in f:
				if isFirstLine == True:
					isFirstLine = False
					continue
	...
	for i in range(0, len(lstV)):
		dis, count = computeDistanceEuclide(lstV[i].getVector(), vector, count)
